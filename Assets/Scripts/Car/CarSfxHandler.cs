...
public class CarSfxHandler : MonoBehaviour
{
	...
	private void UpdateEngineSfx()
	{
		...
		_audioManager.EventInstances[(int)AudioNameEnum.CarEngine]
			.setVolume(Mathf.Lerp(volume, desireEngineVolume, Time.deltaTime * 10));
	...
	private void UpdateTiresScreechingSfx()
	{
		if (_carController.IsTireScreeching(out var lateralVelocity, out var isBraking))
		{
			if (isBraking)
			{
				...
				_audioManager.EventInstances[(int)AudioNameEnum.CarSkid]
					.setVolume(Mathf.Lerp(volume, 1.0f, Time.deltaTime * 10));
		...
		else
		{
			_audioManager.EventInstances[(int)AudioNameEnum.CarSkid].getVolume(out var volume);
			_audioManager.EventInstances[(int)AudioNameEnum.CarSkid]
				.setVolume(Mathf.Lerp(volume, 0, Time.deltaTime * 10));
		}
	}
